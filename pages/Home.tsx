...
import { Wifi, Activity, Play, Filter, ChevronDown, X, Search, Check, Zap, Lock, BatteryCharging, Clock, Share2, Star, Shield, TrendingUp, ArrowRight, Cpu, Eye, BarChart3, Globe, Layers, Flame, Radio, Database, Users } from 'lucide-react';
...
export const Home: React.FC = () => {
  ...
  /* ── Animated stats ── */
  const statProviders = useAnimatedCounter(50, 1800);
  const statGames = useAnimatedCounter(10000, 2200);
  const statLive = useAnimatedCounter(24, 1500);
  ...
  const stats = [
    { label: "Providers", value: 50, suffix: "+", icon: Radio, ref: statProviders.ref, count: statProviders.count },
    { label: "Games", value: 10000, suffix: "+", icon: Database, ref: statGames.ref, count: statGames.count },
    { label: "Live", value: 24, suffix: "/7", icon: Activity, ref: statLive.ref, count: statLive.count },
    { label: "Real-time", value: null, suffix: "", icon: Zap, ref: null, count: null },
  ];
      ...
      <div className="px-4 max-w-4xl mx-auto">
        ...
        <div className="text-center mb-16 pt-8 md:pt-16">
          ...
          <div className="hero-element mb-12">
            <a href="#scanner" className="inline-flex items-center gap-3 px-5 py-2.5 rounded-full bg-neutral-900 border border-neutral-800 hover:border-accent-500/30 transition-all pill-glow group">
              <span className="px-2 py-0.5 rounded-md bg-accent-500 text-white text-[10px] font-bold uppercase animate-pulse">Live</span>
        ...
        <div className="hero-element mb-16">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {stats.map((stat, i) => (
              <div key={i} className="bg-[#111111] border border-neutral-800 rounded-2xl p-4 text-center card-hover group">
                <stat.icon size={20} className="text-accent-500 mx-auto mb-2 group-hover:scale-110 transition-transform" />
                {stat.ref ? (
                  <span ref={stat.ref} className="text-2xl md:text-3xl font-black text-white font-mono block">
                    {stat.count?.toLocaleString()}{stat.suffix}
                  </span>
                ) : (
                  <span className="text-2xl md:text-3xl font-black text-accent-500 font-mono block flex items-center justify-center gap-1">
                    <Activity size={20} className="animate-pulse" /> LIVE
                  </span>
                )}
                <span className="text-[10px] uppercase font-mono text-neutral-600 tracking-wider mt-1 block">{stat.label}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* ═══════════════════════════════════════
            SCANNER SECTION
        ═══════════════════════════════════════ */}
        <div id="scanner" className="mb-16 scroll-mt-20">
          ...
          <div className="hero-element bg-[#111111]/80 border border-neutral-800 rounded-3xl p-6 md:p-8 backdrop-blur-sm">
            
            {/* Provider Selector */}
            <div className="max-w-md mx-auto mb-6 relative z-20">
              <label className="text-xs font-mono font-medium text-neutral-600 uppercase tracking-wider mb-2 block text-center">
              ...
              <button
                ...
                className={`w-full flex items-center justify-between gap-4 border font-semibold py-3.5 px-5 rounded-xl transition-all duration-300 group
                  ${isScanning ? 'opacity-60 cursor-not-allowed border-neutral-800 bg-[#0d0d0d] text-neutral-500' :
                    selectedProvider ? 'bg-[#0d0d0d] border-accent-500/40 text-white hover:border-accent-500/60 shadow-[0_0_20px_rgba(255,51,51,0.05)]' :
                      'bg-[#0d0d0d] border-neutral-700 text-neutral-400 hover:border-accent-500/30'
                  }`}
              >
                <div className="flex items-center gap-3">
                  <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold transition-all
                    ${selectedProvider ? 'bg-accent-500 text-white shadow-[0_0_10px_rgba(255,51,51,0.3)]' : 'bg-neutral-800 text-neutral-500'}`}>
            ...
            {isRestricted && (
              <div className="max-w-md mx-auto mb-6">
                <div className={`bg-[#0d0d0d] rounded-xl p-1 border transition-all duration-300 ${inputError ? 'border-accent-500/50 shadow-[0_0_15px_rgba(255,51,51,0.1)]' : 'border-neutral-800'}`}>
                  <div className="relative flex items-center">
                    ...
                    <input
                      ...
                      disabled={isScanning}
                      className="w-full pl-11 pr-4 py-3 bg-transparent text-white font-medium placeholder:text-neutral-600 focus:outline-none text-center rounded-xl text-sm font-mono"
                    />
                  </div>
                </div>
              ...
              <div className="w-full max-w-md">
                ...
                <button
                  ...
                  className={`relative w-full flex items-center justify-center gap-3 px-8 py-4 rounded-xl font-bold text-base overflow-hidden transition-all duration-300
                    ${isMaintenance ? 'bg-[#0d0d0d] text-neutral-500 cursor-not-allowed border border-neutral-800' :
                      isScanning ? 'cursor-not-allowed bg-[#0d0d0d] text-white border border-neutral-800' :
                        'bg-accent-500 text-white hover:bg-accent-600 shadow-lg shadow-accent-500/25 border border-accent-400/20 hover:shadow-accent-500/40'
                    }`}
                >
                  ...
                  {isScanning && (
                    <div className="absolute inset-y-0 left-0 bg-accent-600/50 transition-all duration-300 rounded-xl" style={{ width: `${progress}%` }}></div>
                  )}
                  <div className="relative z-10 flex items-center gap-2">
                    {isScanning ? (
                      <><Activity className="animate-spin" size={18} /><span className="tracking-wide font-mono">{t('hero.scanning')} {Math.round(progress)}%</span></>
                    ) : isMaintenance ? (
        ...
        {results.length > 0 && (
          <div ref={resultsRef} className="mt-12 mb-16">
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="flex items-center gap-3">
                <span className="text-xs font-mono text-neutral-700 bg-[#111111] border border-neutral-800 px-2 py-1 rounded hidden sm:inline-block">
                  ID: {Date.now().toString().slice(-6)}
                </span>
                <button onClick={handleShare} className="flex items-center gap-2 px-4 py-2 bg-accent-500 text-white text-xs font-bold rounded-lg hover:bg-accent-600 transition-colors shadow-lg shadow-accent-500/20">
                  <Share2 size={14} /> {t('results.share')}
                </button>
              </div>
            </div>

            {/* Results as cards grid */}
            ...
            {PROVIDERS_LIST.slice(0, 20).map((p, i) => (
              <button
                ...
                className={`px-4 py-2 rounded-full text-sm font-medium transition-all duration-200
                  ${selectedProvider === p 
                    ? 'bg-accent-500/10 border border-accent-500/30 text-accent-500 shadow-[0_0_10px_rgba(255,51,51,0.05)]' 
                    : 'bg-[#111111] border border-neutral-800 text-neutral-500 hover:border-accent-500/20 hover:text-white'
                  }`}
              >
                {p}
              </button>
        ...
        {/* ═══════════════════════════════════════
            TELEGRAM CTA — (replaces newsletter)
        ═══════════════════════════════════════ */}
        <div className="hero-element mb-16">
          <div className="bg-[#111111] border border-neutral-800 rounded-3xl p-8 md:p-12 text-center relative overflow-hidden">
            {/* Background glow */}
            <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[400px] h-[200px] bg-accent-500/5 blur-[100px] rounded-full pointer-events-none"></div>
            <div className="relative z-10">
              <div className="inline-flex items-center justify-center w-14 h-14 rounded-2xl bg-accent-500/10 border border-accent-500/20 mb-4">
                <Radio size={24} className="text-accent-500" />
              </div>
              <h2 className="text-xl md:text-2xl font-bold text-white mb-2">Stay in the Loop</h2>
              <p className="text-neutral-500 text-sm mb-6 max-w-sm mx-auto">Join our Telegram for real-time RTP alerts, new providers, and updates.</p>
              <a
                href="https://t.me/slotdatartp"
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-2 px-6 py-3 bg-accent-500 text-white font-bold text-sm rounded-xl hover:bg-accent-600 transition-colors shadow-lg shadow-accent-500/20 hover:shadow-accent-500/30 hover:-translate-y-0.5 transition-all duration-200"
              >
                Join Telegram <ArrowRight size={14} />
              </a>
            </div>
          </div>
        </div>
        ...
        {showProviderModal && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <div className="relative w-full max-w-2xl bg-[#111111] border border-neutral-800 rounded-2xl shadow-2xl overflow-hidden max-h-[85vh] flex flex-col">
              ...
              <div className="px-6 py-3 border-b border-neutral-800">
                <div className="relative">
                  ...
                  <input type="text" placeholder="Search provider..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full bg-[#0a0a0a] border border-neutral-800 rounded-xl py-2.5 pl-10 pr-4 text-sm text-white placeholder:text-neutral-600 focus:outline-none focus:border-accent-500/30 font-mono" autoFocus />
                </div>
              </div>
              <div className="p-4 overflow-y-auto flex-1">
                  ...
                  {filteredProviders.map((p) => (
                    <button key={p} onClick={() => { setSelectedProvider(p); setShowProviderModal(false); }}
                      className={`relative flex items-center gap-2 p-3 rounded-xl border transition-all duration-200 text-left
                        ${selectedProvider === p ? 'bg-accent-500/10 border-accent-500/40 text-white shadow-[0_0_15px_rgba(255,51,51,0.05)]' : 'bg-[#0a0a0a] text-neutral-400 border-neutral-800 hover:border-accent-500/20 hover:text-white'}`}>
                      <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold flex-shrink-0 ${selectedProvider === p ? 'bg-accent-500 text-white shadow-[0_0_8px_rgba(255,51,51,0.3)]' : 'bg-neutral-800 text-neutral-500'}`}>
                        {p.charAt(0)}
                      </div>
                ...
                {filteredProviders.length === 0 && <div className="text-center py-8 text-neutral-600 font-mono">No providers found</div>}
              </div>
            </div>
          </div>
        )}
